...
# kubectl get node  -owide --no-headers  |awk '{print $6}'
k8s_hosts=(
10.25.27.12
10.25.27.15
10.25.27.9
...
gpaasSecurityChain="gpaas-container-security"
...
addHarborIptables(){
	for host in ${harbor_hosts[@]}; do
		commonSet
		ssh $host "cd  /tmp/setiptables;chmod +x set-iptables-harbor.sh;  ./set-iptables-harbor.sh"
	done
}

addBigdataIptables(){
	for host in ${bigdata_hosts[@]}; do
		commonSet
	done
}
...
commonSet(){
	  ...
	  ssh $host "cd  /tmp/setiptables;chmod +x set.sh;  ./set.sh ${gpaasSecurityChain}"
}

updateMaster443(){
	port=443

	rancher_ip=$(docker inspect --format '{''{ .NetworkSettings.IPAddress }''}' rancher_server)
